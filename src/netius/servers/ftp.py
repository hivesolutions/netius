...
class FTPConnection(netius.Connection):
    ...
    def send_ftp(
        self, code, message="", lines=(), simple=False, delay=True, callback=None
    ):
        if lines:
            return self.send_ftp_lines(
                code,
                message=message,
                lines=lines,
                simple=simple,
            ...
            return self.send_ftp_base(code, message, delay, callback)
    ...
    def send_ftp_lines(
        ...
        lines_s = (
            ...
            if simple
            else ["%d-%s" % (code, line) for line in body]
        )
        lines_s.append("%d %s" % (code, tail))
        if simple:
            lines_s.insert(0, base)
        data = "\r\n".join(lines_s) + "\r\n"
        count = self.send(data, delay=delay, callback=callback)
        self.owner.debug(base)
        return count
    ...
    def on_cwd(self, message):
        ...
        if is_absolute:
            ...
            cwd = self.cwd + (message if self.cwd.endswith("/") else "/" + message)

        full_path = self._get_path(extra=message)
        is_dir = os.path.isdir(full_path)
        if not is_dir:
            self.send_ftp(550, "failed to change directory")
            return
    ...
    def _data_open(self):
        ...
        self.data_server.serve(host=self.owner.host, port=0, load=False, start=False)
    ...
    def _to_unix(self, mode):
        ...
        return is_dir + "".join(
            [PERMISSIONS.get(int(item), item) for item in permissions]
        )
...
class FTPDataServer(netius.StreamServer):
    ...
    def send_ftp(self, data, delay=True, force=False, callback=None):
        if not self.accepted:
            raise netius.DataError("No connection accepted")
